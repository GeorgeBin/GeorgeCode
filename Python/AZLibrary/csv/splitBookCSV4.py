...
# 定义错误文件路径
error_file = os.path.join(input_root, 'split_err.csv')


# 初始化一个空的文件字典，用于存储分割后的数据
output_files = {filename: open(os.path.join(input_root, filename), 'w', newline='') for _, _, filename in ranges}

# 打开输入文件和错误文件
with open(input_file, 'r', newline='') as csvfile, open(error_file, 'w', newline='') as errfile:
    ...
    # 遍历输入文件的每一行
    for row in reader:
        # 将 zlibrary_id 转换为整数，如果无法转换，则将其写入错误文件
        try:
            row['zlibrary_id'] = int(row['zlibrary_id'])
        except ValueError:
            writer_err.writerow([row['zlibrary_id'], row['extension'], row['title']])
            continue

        # 根据范围确定要写入的输出文件
        for start, end, filename in ranges:
            if start <= row['zlibrary_id'] <= end:
                writer = csv.DictWriter(output_files[filename], fieldnames=reader.fieldnames)
                writer.writerow(row)
                break
